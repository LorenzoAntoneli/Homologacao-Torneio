...
export const AdminProvider = ({ children }) => {
  ...
  const notifyTV = () => {
    if (tvChannel) {
      tvChannel.send({ type: 'broadcast', event: 'sync_data', payload: { ts: Date.now() } });
    }
  };
  ...
  const createTournament = async () => { if (!newTName) return; const { error } = await supabase.from('tournaments').insert([{ name: newTName }]); if (error) alert("Erro ao criar torneio: " + error.message); else { setNewTName(''); loadData(); notifyTV(); } };
  const createCategory = async () => { if (!selectedT || !newCName) return; const { error } = await supabase.from('categories').insert([{ tournament_id: selectedT, name: newCName }]); if (error) alert("Erro ao criar categoria: " + error.message); else { setNewCName(''); loadData(); notifyTV(); } };
  const createCourt = async () => { if (!selectedT || !newCourtName) return; const { error } = await supabase.from('courts').insert([{ tournament_id: selectedT, name: newCourtName }]); if (error) alert("Erro ao criar quadra: " + error.message); else { setNewCourtName(''); loadData(); notifyTV(); } };
  const createSponsor = async () => { if (!newSponsor.name || !newSponsor.logo_url) return; const { error } = await supabase.from('sponsors').insert([newSponsor]); if (error) alert("Erro ao criar patrocinador: " + error.message); else { setNewSponsor({ name: '', logo_url: '' }); loadData(); notifyTV(); } };
  ...
  const deleteSponsor = async (id) => { if (!window.confirm('Excluir este patrocinador?')) return; const { error } = await supabase.from('sponsors').delete().eq('id', id); if (error) alert(error.message); else loadData(); };
  const createPair = async () => { if (!selectedC || !atleta1 || !atleta2) return; const { error } = await supabase.from('pairs').insert([{ category_id: selectedC, name: `${atleta1} / ${atleta2}` }]); if (error) alert("Erro ao criar dupla: " + error.message); else { setAtleta1(''); setAtleta2(''); loadData(); notifyTV(); } };
  const createMatch = async () => { if (!selectedT || !selectedC || !matchP1 || !matchP2) return; const { error } = await supabase.from('matches').insert([{ tournament_id: selectedT, category_id: selectedC, pair1_id: matchP1 || null, pair2_id: matchP2 || null, court_id: matchCourt || null, scheduled_time: matchTime || null, status: 'pending' }]); if (error) alert("Erro ao criar partida: " + error.message); else { loadData(); setActiveTab('scoreboard'); notifyTV(); } };
  const deleteMatch = async (id) => { if (!window.confirm('⚠️ Tem certeza que deseja APAGAR esta partida?')) return; const { error } = await supabase.from('matches').delete().eq('id', id); if (error) alert(error.message); else { loadData(); notifyTV(); } };
  ...
  const executeImport = async () => { if (!selectedT || !importData || importData.length === 0) return; setIsImporting(true); try { const uniqueCats = [...new Set(importData.map(d => d.categoria))]; for (const catName of uniqueCats) { const exists = categories.find(c => c.name === catName && c.tournament_id === selectedT); if (!exists) { await supabase.from('categories').insert([{ tournament_id: selectedT, name: catName }]); } } const { data: freshCats } = await supabase.from('categories').select('*').eq('tournament_id', selectedT); for (const row of importData) { const cat = freshCats.find(c => c.name === row.categoria); if (!cat) continue; const pairName = `${row.atleta1} / ${row.atleta2}`; let { data: pair } = await supabase.from('pairs').select('id').eq('category_id', cat.id).eq('name', pairName).single(); if (!pair) { const { data: newPair } = await supabase.from('pairs').insert([{ category_id: cat.id, name: pairName }]).select().single(); pair = newPair; } }
    alert('✅ Importação concluída com sucesso!'); setImportData(null); setImportFileName(''); loadData(); notifyTV(); } catch (err) { alert('Erro na importação: ' + err.message); } finally { setIsImporting(false); } };
